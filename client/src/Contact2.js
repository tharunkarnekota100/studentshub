...
const Contact2 = () => {
    ...
    const problemHandler = e =>{
        e.preventDefault();
        if( problem.length>5 ){
            axios.post('http://localhost:5000/addquerybme',info,{
                    headers : {
    ...
    const contactsubmithandler = e =>{
        ...
        {
            axios.post('http://localhost:5000/addsupportteambme',contact,{
                    headers : {
                        'x-token' : localStorage.getItem('token')
                    }
                }).then(res => alert(res.data))
    ...
    useEffect(()=>{
        ...
        axios.get('http://localhost:5000/getsupportteambme',{
            headers : {
                'x-token' : localStorage.getItem('token')
            }
        }).then(res => setSupportteam(res.data))

        axios.get('http://localhost:5000/getpresentuserbme',{
            headers : {
                'x-token' : localStorage.getItem('token')
            }
